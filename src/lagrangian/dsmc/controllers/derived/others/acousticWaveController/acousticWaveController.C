...
#include "acousticWaveController.H"
...
namespace Foam
{
    defineTypeNameAndDebug(acousticWaveController, 0);

    addToRunTimeSelectionTable(dsmcStateController, acousticWaveController, dictionary);

    // * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

    acousticWaveController::acousticWaveController
    (
        Time& t,
        dsmcCloud& cloud,
    ...
    void acousticWaveController::initialConfiguration()
    {
        initialXPositions_.setSize(cloud_.size());
    }

    void acousticWaveController::calculateProperties()
    {
    }

    void acousticWaveController::controlParcelsBeforeMove()
    {
        nTimeSteps_++;
    ...
    void acousticWaveController::output
        (
    ...
    void acousticWaveController::controlParcelsBeforeCollisions()
    {
        forAll(controlZone(), c)
        {
            const List<DynamicList<dsmcParcel*> >& cellOccupancy = cloud_.cellOccupancy();
    ...
    void acousticWaveController::controlParcelsAfterCollisions()
    {
        forAll(controlZone(), c)
        {
            const List<DynamicList<dsmcParcel*> >& cellOccupancy = cloud_.cellOccupancy();
    ...
    void acousticWaveController::updateProperties(const dictionary& newDict)
    {
    ...
    void acousticWaveController::setProperties()
    {       
        accelerationDirection_ = propsDict_.lookup("accelerationDirection");
        waveNumber_ = readScalar(propsDict_.lookup("waveNumber"));
        frequency_ = readScalar(propsDict_.lookup("frequency"));
