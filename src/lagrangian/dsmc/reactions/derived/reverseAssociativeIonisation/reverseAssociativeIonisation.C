...
void reverseAssociativeIonisation::setProperties()
{
    ...
    const List<word> productMolecules
    ...
    forAll(productIds_, i)
    {
        ...
        scalar rDof =
               cloud_.constProps(productIds_[i]).rotationalDegreesOfFreedom();
...
bool reverseAssociativeIonisation::tryReactMolecules(const label typeIdP,
                                                     const label typeIdQ)
{
...
)
{
    ...
    if (typeIdP == reactantIds_[0] && typeIdQ == reactantIds_[1])
    {
        ...
        scalar Ec = reactionProperties(cloud_, p, q).translationalEnergy() + reactionProperties(cloud_, p, q).EEleP();
        scalar EcOrig = Ec;
        ...
        if (ELevelIntermediate == 0)
        {
            ...
            scalar EVib = eVibLevel*physicoChemical::k.value()
                                *thetaVIntermediate;
    ...
    if (typeIdP == reactantIds_[1] && typeIdQ == reactantIds_[0])
    {
        ...
        scalar Ec = reactionProperties(cloud_, p, q).translationalEnergy() + reactionProperties(cloud_, p, q).EEleQ();
        scalar EcOrig = Ec;
...
void  reverseAssociativeIonisation::outputResults(const label counterIndex)
{
    if (writeRatesToTerminal_ == true)
    {
        ...
        const word& productMolA = cloud_.typeIdList()[productIds_[0]];
        const word& productMolB = cloud_.typeIdList()[productIds_[1]];
...
bool reverseAssociativeIonisation::relax() const
{
    return relax_;
}
